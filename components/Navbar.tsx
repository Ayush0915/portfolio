  ...
  return (
    <header className="fixed top-5 left-1/2 z-50 w-full -translate-x-1/2 px-4">
      ...
      <nav className="hidden md:flex mx-auto max-w-fit items-center gap-1 rounded-full border border-zinc-700/60 bg-zinc-900/80 px-4 py-2 shadow-lg shadow-black/40 backdrop-blur-md">
        <Link
          href="/"
          className="mr-2 px-3 py-1.5 text-sm font-bold tracking-tight text-zinc-100 transition-colors hover:text-indigo-400 min-h-[44px] flex items-center"
        >
          AB
        </Link>
              ...
              {navLinks.map((link, index) => (
                ...
                >
                  <a
                    href={link.href}
                    onClick={() => setIsOpen(false)}
                    className="block px-5 py-3.5 text-base font-medium text-zinc-300 transition-colors hover:bg-zinc-800/80 hover:text-zinc-100 border-b border-zinc-800/50 min-h-[44px] flex items-center"
                  >
                    {link.label}
                  </a>
                </motion.div>
